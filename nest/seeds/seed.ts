 ...
 * export async function seed<
  ...
  K extends import("../src/auth/auth-jwt.service").AuthService,
  D extends import("../src/password/password.service").PasswordService
>(prisma: T, authService: K, passwordService: D) {
  enum Gender {
    MALE = "MALE",
    FEMALE = "FEMALE",
    UNCERTAIN = "UNCERTAIN",
  ...
  const Jwt = (await import("../src/auth/enums/jwt-signing-algorithm.enum"))
    .JwtSigningAlgorithm;
  interface JwtDto {
    header: {
      alg: keyof typeof Jwt;
      typ: string;
    };
    payload: {
      userId: string;
  ...
  const hashPassword = async (input: string): Promise<string> => {
    const hashedPw = passwordService.hashPassword(input);
    return hashedPw;
  };
  const seedUserId = faker.datatype.uuid();

  const generateTokens = (payload: { userId: string }) => {
    return authService.generateTokens({ userId: payload.userId });
  };

  const { accessToken, refreshToken } = generateTokens({ userId: seedUserId });

  const getJwtInfo = async (token: string) => {
    return (await import("@nestjs/jwt")).JwtService.prototype.decode(
      token
    ) as JwtDto;
  };

  const jwtData = await getJwtInfo(accessToken ? accessToken : "");

  const hashedPassword = await hashPassword(
    process.env.REDIS_PASSWORD ? process.env.REDIS_PASSWORD : "Nest2022!"
  );
  const seedFirstName = faker.name.firstName();
  const seedSurname = faker.name.lastName();
          ...
          create: [
            {
              ...
              access_token: accessToken,
              refresh_token: refreshToken,
              refresh_secret: jwtData.signature,
              expires_at: jwtData.payload.exp
            }
          ]
        },
        sessions: {
          create: [
            {
              exp: new Date(jwtData.payload.exp),
              accessToken: accessToken,
              refreshToken: refreshToken,
...
async function main() {
  const PrismaClient = (await import("@prisma/client")).PrismaClient;
  const PasswordService = (await import("../src/password/password.service"))
    .PasswordService;
  const AuthService = (await import("../src/auth/auth-jwt.service"))
    .AuthService;
  const prisma = new PrismaClient({ log: ["error", "info", "query", "warn"] });
  try {
    await prisma
    ...
    const s = async (): Promise<SeedInferred> =>
      await seed(prisma, AuthService.prototype, PasswordService.prototype).then(
        data => {
          console.log(
            `[seeding]: success 🎉 created ${data.role} with id ${data.id} and email ${data.email} -- in country ${data.profile?.country} having phone number ${data.profile?.phoneNumber} -- gender: ${data.profile?.gender}; pronouns: ${data.profile?.pronouns} -- authored ${data.entries[0].title} having id ${data.entries[0].id}`
          );
